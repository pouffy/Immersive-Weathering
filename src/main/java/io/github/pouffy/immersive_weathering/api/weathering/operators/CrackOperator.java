...
public class CrackOperator implements SpreaderOperator, NeighborUpdateOperator, RandomTickOperator {
    @Override
    public Holder<WeatheringOperator> getType() {
        return ModWeatheringOperators.CRACK;
    }

    @Override
    public Optional<PatchSpreader> getPatchSpreader(Holder<? extends PatchSpreader> spreader) {
        if (spreader == ModSpreaders.CRACK) {
    ...
    public void randomTick(BlockState state, ServerLevel serverLevel, BlockPos pos, RandomSource random) {
        if (random.nextFloat() < this.getWeatherChanceSpeed()) {
            ...
            if (ModSpreaders.CRACK.get().getWantedWeatheringState(true, pos, serverLevel)) {
    ...
    public boolean canTarget(BlockState state) {
        ...
        return DataMapHelpers.canAdvance(DataMapHelpers.Type.CRACK, state);
    }
    ...
    public boolean shouldWeather(BlockState state, BlockPos pos, Level level) {
        ...
        return ModSpreaders.CRACK.get().getWantedWeatheringState(false, pos, level);
    }
